...
The second-order blocks – generated by applying some transformations to the first order blocks:
...
I have created 32 different classes to solve some type of abstract task using the abstractions described earlier. All of them inherit from 'Predictor' Class.

The general logic of every predictor is described in the pseudo-code below (thus, it can be different for some classes).
...
I also implemented some functionality, helping my predictors to solve a broader range of tasks. The way I applied them does not strictly qualify as decorators or callbacks, have similar ideas behind them.

## 3.1 Skip some train samples.
If you pass to any predictor {"skip_train": n}, it will skip n input-output train samples. It helps in a couple of cases:
- One of the train samples has a mistake in it.
- Test samples can be divided into 2+ categories, the predictor can solve the task only for one of them, and you are lucky, and the test is also from this group.
...
## 4.3 Multiprocessing
Fast iterations are essential in this competition. That is why I have used multiprocessing wherever it is possible. I have used 24 threads locally for experiments and all four threads in kernels for faster submissions.
...
You can check the way I have done in `submission_utils.py`.
...
## 5.1 Testing
This is the first Kaggle competition where I have covered almost everything I have done with a unit test.
